...
export default class ColumnProperties {
	...
	#move_column() {
		let prop = $(`<div class="btn-group" role="group">
	...
	#set_name() {
		let prop = $(`<input type="text" class="form-control" placeholder="title">`)
			.val(this.ctrl.title)
		...
		if (this.TableManager.system_fields.indexOf(this.ctrl.name) != -1) {
			prop.attr('disabled', 'disabled');
			prop.attr('title', 'System values are not editable');
		}
		return prop;
	}
	...
	#set_datatype() {
		...
		let prop = $(`<select class="form-control" placeholder="data type></select>`)
			.val(this.ctrl.comment)
	...
	#set_viewtype() {
		let types = ['', 'count', 'sum', 'avg', 'max', 'min'];

		let prop = $(`<select class="form-control" placeholder="aggregate></select>`)
			.val(this.ctrl.comment)
			.on('input', this.ctrl, evt => {
	...
	#set_data_len() {
		let prop = $(`<input type="number" min=0 class="form-control" placeholder="data length">`)
			.val(this.ctrl.title)
			.on('input', this.ctrl, evt => {
	...
	#set_default() {
		let prop = $(`<input type="number" class="form-control" placeholder="default value">`)
			.val(this.ctrl.title)
			.on('input', this.ctrl, evt => {
	...
	#set_foreign_key(widget) {
		let $this = this;
		let table_select = $(`<select class="form-control" placeholder="foreign table" ></select>`),
		...
		let _fk_table_changed = function () {
			let opt = null,
				field = null,
				f = null;
			...
			let sql_table = opt.prop('sql_table');

			//REMOVE THE EXISTING RELATION IF ONE EXISTS
			if (!sql_table) {
				// let table = $this.ownerfield;
				for (let k in $this.TableManager.relations) {
					let rel = $this.TableManager.relations[k];
		...
		field_select.on('change', function () {
			let opt = table_select.find('option:selected');
			let sql_table = opt.prop('sql_table');

			let field = $(this).find('option:selected').prop('field');
		...
		display_select.on('change', function () {
			let field = field_select.find('option:selected').prop('field');
			let fk = $(this).find('option:selected').prop('field');
