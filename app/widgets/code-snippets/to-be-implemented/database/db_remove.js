var control_db_remove = {
	type				: 'db_remove',
	control_label		: 'Database Remove Record',
	control_category	: 'Code Snippet',
	control_thumbnail	: 'images/widgets/code-snippet.png',
	description			: 'remove a db record into a table',
	...
	_get_settings: function(){
		...
		var btn = $('<button type="button">remove by</button>').addClass('btn btn-light btn-flat');
		settings.push(['datasource', datasource]);
		settings.push(['entity', tables]);
		settings.push(['map fields to remove by',btn]);
	...
	execute: function(){
		...
		table.remove(record, function(data, err){
			if (err.length !== 0){
				return open_card(err.join('<br>'),{title:'Remove Failed'});
			}
