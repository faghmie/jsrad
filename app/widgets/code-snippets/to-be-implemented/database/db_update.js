var control_db_update = {
	type				: 'db_update',
	control_label		: 'Database Update Record',
	control_category	: 'Code Snippet',
	control_thumbnail	: 'images/widgets/code-snippet.png',
	description			: 'update a db record on a table',
	...
	open_mapper: function(){
		...
		for(key = 0; key < fields.length; key++){
			...
			if (col.required === true){
				required = ' *';
			} else if (col.primary_key === true){
				required = ' <PK>';
			}
			
			field.find('input').before('<label>'+ col.title + required +'</label>');
			mapper.append(field);
		}
	...
	execute: function(){
		...
		for(var f in this.message_map){
			if (['datasource', 'entity'].indexOf(f) !== -1) continue;
			
			record[f] = this.get_attribute(f);
		}
		
		table.update(null, record, function(data, errors){
			if (errors.length !== 0){
				App.MessageError(errors);
				$this.next();
				return;
			}
